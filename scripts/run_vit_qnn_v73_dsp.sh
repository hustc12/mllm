...
if ! adb shell [ -f "/data/local/tmp/mllm/models/vit-base-patch16-224-fp32.mllm" ]; then
    adb push ../models/vit-base-patch16-224-fp32.mllm "/data/local/tmp/mllm/models/vit-base-patch16-224-fp32.mllm"
else
    echo "vit-base-patch16-224-fp32.mllm file already exists"
fi


if ! adb shell [ -f "/data/local/tmp/mllm/models/vit-base-patch16-224-q4_k.mllm" ]; then
    adb push ../models/vit-base-patch16-224-q4_k.mllm "/data/local/tmp/mllm/models/vit-base-patch16-224-q4_k.mllm"
else
...
adb push ../bin-arm/demo_vit_dsp /data/local/tmp/mllm/bin/
adb shell "cd /data/local/tmp/mllm/bin && export LD_LIBRARY_PATH=/data/local/tmp/mllm/qnn-lib && export ADSP_LIBRARY_PATH=/data/local/tmp/mllm/qnn-lib && ./demo_vit_dsp"
