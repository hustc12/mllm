...
if ! adb shell [ -f "/data/local/tmp/mllm/models/yolox_x.mllm" ]; then
    adb push ../models/yolox_x.mllm "/data/local/tmp/mllm/models/yolox_x.mllm"
else
    echo "yolox_x.mllm file already exists"
fi
...
adb push ../bin-arm/demo_yolox_x /data/local/tmp/mllm/bin/
adb shell "cd /data/local/tmp/mllm/bin && export LD_LIBRARY_PATH=/data/local/tmp/mllm/qnn-lib && export ADSP_LIBRARY_PATH=/data/local/tmp/mllm/qnn-lib && ./demo_yolox_x"